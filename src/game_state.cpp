...
bool game_state_update_config(const DeviceConfig* new_config) {
    ...
    ESP_LOGI(TAG, "Config updated - ID: %s, Name: %s, Team: %s", 
             s_config.device_id, s_config.device_name, s_config.team);
...
int game_state_config_to_json(char* buffer, size_t max_len) {
    ...
    int len = snprintf(buffer, max_len,
        ...
        "\"%s\":%lu"
        "}",
        JSON_KEY_DEVICE_ID, s_config.device_id,
        JSON_KEY_DEVICE_NAME, s_config.device_name,
        JSON_KEY_PLAYER_ID, s_config.player_id,
        JSON_KEY_ROLE, DEVICE_ROLE_NAMES[s_config.role],
        JSON_KEY_TEAM, s_config.team,
        JSON_KEY_COLOR, s_config.color,
        JSON_KEY_TEAMMATES, s_config.teammates,
        JSON_KEY_ENEMIES, s_config.enemies,
        JSON_KEY_RESPAWN_TIME, (unsigned long)s_config.respawn_time_ms
    );
    UNLOCK();
...
int game_state_to_json(char* buffer, size_t max_len) {
    ...
    int len = snprintf(buffer, max_len,
        ...
        "\"%s\":%s"
        "}",
        JSON_KEY_GAMEMODE, GAMEMODE_NAMES[s_game_state.mode],
        JSON_KEY_GAME_STATE, GAME_STATE_NAMES[s_game_state.state],
        JSON_KEY_KILLS, (unsigned long)s_game_state.kills,
        JSON_KEY_DEATHS, (unsigned long)s_game_state.deaths,
        JSON_KEY_SHOTS, (unsigned long)s_game_state.shots_fired,
        JSON_KEY_HITS, (unsigned long)s_game_state.hits_landed,
        JSON_KEY_HEALTH, s_game_state.health,
        "server_connected", s_game_state.server_connected ? "true" : "false"
    );
    UNLOCK();
...
int game_state_create_heartbeat_json(char* buffer, size_t max_len) {
    ...
    int len = snprintf(buffer, max_len,
        ...
        "\"%s\":%lu"
        "}",
        JSON_KEY_TYPE, CLIENT_MSG_NAMES[MSG_HEARTBEAT],
        JSON_KEY_DEVICE_ID, s_config.device_id,
        JSON_KEY_GAMEMODE, GAMEMODE_NAMES[s_game_state.mode],
        JSON_KEY_GAME_STATE, GAME_STATE_NAMES[s_game_state.state],
        JSON_KEY_KILLS, (unsigned long)s_game_state.kills,
        ...
        JSON_KEY_HITS, (unsigned long)s_game_state.hits_landed,
        JSON_KEY_HEALTH, s_game_state.health,
        JSON_KEY_UPTIME, (unsigned long)uptime
    );
    UNLOCK();
...
int game_state_create_register_json(char* buffer, size_t max_len) {
    ...
    int len = snprintf(buffer, max_len,
        ...
        "\"%s\":\"%s\""
        "}",
        JSON_KEY_TYPE, CLIENT_MSG_NAMES[MSG_REGISTER],
        JSON_KEY_DEVICE_ID, s_config.device_id,
        JSON_KEY_DEVICE_NAME, s_config.device_name,
        JSON_KEY_PLAYER_ID, s_config.player_id,
        JSON_KEY_ROLE, DEVICE_ROLE_NAMES[s_config.role],
...
int game_state_create_hit_report_json(char* buffer, size_t max_len, const char* shooter_id) {
    ...
    int len = snprintf(buffer, max_len,
        ...
        "\"%s\":%lu"
        "}",
        JSON_KEY_TYPE, CLIENT_MSG_NAMES[MSG_HIT_REPORT],
        JSON_KEY_TARGET_ID, s_config.device_id,
        JSON_KEY_SHOOTER_ID, shooter_id ? shooter_id : "unknown",
        JSON_KEY_TIMESTAMP, (unsigned long)(esp_timer_get_time() / 1000)
    );
    UNLOCK();
...
int game_state_create_shot_fired_json(char* buffer, size_t max_len) {
    ...
    int len = snprintf(buffer, max_len,
        ...
        "\"%s\":%lu"
        "}",
        JSON_KEY_TYPE, CLIENT_MSG_NAMES[MSG_SHOT_FIRED],
        JSON_KEY_DEVICE_ID, s_config.device_id,
        JSON_KEY_SHOTS, (unsigned long)s_game_state.shots_fired,
        JSON_KEY_TIMESTAMP, (unsigned long)(esp_timer_get_time() / 1000)
    );
...
bool game_state_config_from_json(const char* json, DeviceConfig* out_config) {
    ...
    if (!json || !out_config) return false;
